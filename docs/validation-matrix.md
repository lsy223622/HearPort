...
| ios-quic | Network.framework QUIC client, one control stream and one DATAGRAM flow | not-available | macOS/Xcode/iPadOS SDK is unavailable on this host; `.github/workflows/ios-receiver.yml` runs the Swift package checks on `macos-14`; implementation is in `ios/HearPortReceiver/Sources/HearPortReceiver/QuicReceiver.swift` | pending GitHub Actions/Apple run |
