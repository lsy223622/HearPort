...
- Task 8: complete (commit e5f43d0); diagnostics schemas, onboarding guidance, and evidence matrix added. Tests: validation-matrix checker -> 2/2 passed, full Python reference suite -> 35/35 passed, diagnostic schemas parse as 2 valid JSON documents, `git diff --check` passed. Native Windows/Swift and real-device/network-soak rows are explicitly marked `not-available` or pending in `docs/validation-matrix.md`.
