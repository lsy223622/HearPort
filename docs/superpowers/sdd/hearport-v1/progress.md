...
- Native Windows follow-up: pending commit; added the Windows link interface required by the Rust staticlib (`ws2_32`, `userenv`, `ntdll`), corrected the native identity-size assertions, and included a macOS GitHub Actions Swift package workflow. Final evidence: full Ninja build passed and CTest passed 7/7; MsQuic runtime and Apple/device tests remain pending.
